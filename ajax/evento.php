...
switch ($_GET["op"]){
	...
	case 'guardaryeditar' :
		if (empty($id_evento)){
			...
			$cantidad_detalle_evento = $_POST['cantidad_detalle_evento'] ?? [];
			$precio_detalle_evento = $_POST['precio_detalle_evento'] ?? [];
			$var = $_ ?? "default";
			$respuesta = $evento->insertar(
				$fecha_evento,
				$nombre_evento,
				$tipo_evento,
				$cotizacion_evento,
			...
			echo $respuesta ? "Evento registrado" : "El evento no se pudo registrar";
		}
		elseif ($estado_evento=="Programado") {
			$id_servicio = $_POST['id_servicio'] ?? [];
			$cantidad_detalle_evento = $_POST['cantidad_detalle_evento'] ?? [];
			$precio_detalle_evento = $_POST['precio_detalle_evento'] ?? [];
			$respuesta = $evento->editarEventos(
				$id_evento,
				$fecha_evento,
				$nombre_evento,
				$tipo_evento,
			...
			echo $respuesta="El evento no se pudo editadar porque ya está finalizado";
		} 
	break;
	...
	case 'mostrar' :
		...
		if ($respuesta == null){
			$respuesta['totalFormato'] = 0;
			$respuesta['total_evento'] = 0;
		}
		$respuesta['totalFormato'] = "$ ". number_format($respuesta['total_evento'],2);
	...
	case 'listarDetalle':
		...
		while ($reg = $rspta->fetch_object()){
		   ...
		   echo "<tr class='filas' id=fila".$cont." style='text-align:; vertical-align:middle;'>
					...
					<td><input type='number' class='form-control' name='cantidad_detalle_evento[]' id='cantidad_detalle_evento[]' value='".$reg->cantidad_detalle_evento."'></td>
					<td><input type='number' class='form-control' name='precio_detalle_evento[]' id='precio_detalle_evento[]' value='".$reg->precio_detalle_evento."'></td>
	...
	case 'listarDetalleEvento':
		...
		while ($reg = $rspta->fetch_object()){
		   ...
		   echo "<tr class='filas' id=fila".$cont." style='text-align:; vertical-align:middle;'>
					...
					<td><input type='hidden' name='id_servicio[]' value='".$reg->id_servicio."'>".$reg->nombre_servicio."</td>
					<td><input type='hidden' name='nombre_categoria_servicio[]' id='nombre_categoria_servicio[]'>".$reg->nombre_categoria_servicio."</td>
					<td><input type='number' class='form-control' step='1' name='cantidad_detalle_evento[]' id='cantidad_detalle_evento[]' value='".round($reg->cantidad_detalle_evento,0)."'></td>
					<td><input type='number' class='form-control' name='precio_detalle_evento[]' id='precio_detalle_evento[]' value='".$reg->precio_detalle_evento."'></td>
					<td><span name='subtotal[]' id='subtotal".$cont."'>".$totalAux2."</span></td>
					<td><button type='button' onclick='modificarSubototales()' class='btn btn-info'><i class='fa fa-refresh'></i></button></td>
				</tr>";
	...
	case 'listarTablaAExportar':
		...
		echo "<tfoot>
                ...
                <th></th>
                <th></th>
                <th><h4 id='total' style='text-align: right; vertical-align:middle;' >$".number_format($total, 2).'</h4><input type="hidden" name="" id=""></th> 
            </tfoot>';
	break;
	...
	case 'listar' :
		...
		while ($reg=$respuesta->fetch_object()){
			...
			$data[]=array(
				"0"=>(($reg->estado_evento=='Programado')?'<abbr title="Ver detalle"><button class="btn btn-primary" onclick="mostrar('.$reg->id_evento.')"><i class="fa fa-eye"></i></button></abbr> '.
				'<abbr title="Finalizar evento"><button class="btn btn-danger" onclick="anular('.$reg->id_evento.')"><i class="fa fa-check-square-o"></i></button></abbr> ':
				'<abbr title="Ver detalle"><button class="btn btn-primary" onclick="mostrar('.$reg->id_evento.')"><i class="fa fa-eye"></i></button></abbr> '.
	...
	case 'listarServicios':
		...
		while ($reg=$respuesta->fetch_object()){
			$data[]=array(
				"0"=>'<button class="btn btn-warning" onclick="agregarDetalle('.$reg->id_servicio.',\''.$reg->nombre_servicio.'\',\''.$reg->categoria.'\')"><span class="fa fa-plus"></span></button>',
				"1"=>$reg->categoria,
