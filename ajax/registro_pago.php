...
switch ($_GET["op"])
{
	case 'registrar_pago' :
		...
		if ($respuesta){
			...
			if ( $correo->enviarCorreoPago($email_pago, $_SESSION['email_usuario'], $fecha_pago, 
				$monto_pago, $metodo_pago, $nombre_proveedor, $notas_pago, $nombre_evento_seleccionado) ){
	...
	case 'eliminar_pago' :
		...
		if ($respuesta){
			...
			if ( $correo->enviarCorreoPagoEliminado($email_pago, $_SESSION['email_usuario'], $fecha_pago, 
				$monto_pago, $nombre_proveedor, $metodo_pago, $nombre_evento_seleccionado) ){
				$resultado .= " y correo enviado correctamente.";
			} else {
				$resultado .= ". El pago no pudo ser enviado.";
			}
