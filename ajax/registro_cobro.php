...
switch ($_GET["op"]) {
	...
	case 'listarCobros':
		...
		if (count($listaCobros) == 0) { //Si no hay historial de
			...
			foreach ($listaCobros as $cobro) {
				...
				$metodo_cobro = $aux->asignarMetodoCobro($metodo_cobro);
				$data[] = array(
					"0" => ('<button class="btn btn-primary reenviar"><i class="fa fa-envelope"></i></button>' .
						'<button class="btn btn-danger eliminar"><i class="fa fa-close"></i></button>'
					),
	...
	case 'eliminar_cobro':
		...
		if ($respuesta) {
			...
			if ($correo->enviarCorreoCobroEliminado(
				$email_cobro,
				$_SESSION['email_usuario'],
				$fecha_cobro,
				$monto_cobro,
