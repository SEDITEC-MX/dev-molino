...
switch ($_GET["op"]) {
	...
	case 'listar':
		...
		foreach ($respuesta as $evento) {
			...
			$data[] = array(
				"0" => (($evento['estado_evento'] == 'Programado') ? '<abbr title="Ver detalle"><button class="btn btn-primary" onclick="mostrar(' . $evento['id_evento'] . ')"><i class="fa fa-search fa-xs"></i></button></abbr> ' :
