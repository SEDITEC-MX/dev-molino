...
*/
	...
	public function listarDetalle($id_evento)
	{
		...
		servicio.nombre_categoria_servicio 
		FROM pagos
	...
	{
		$sql="SELECT evento.id_evento, DATE_FORMAT(evento.fecha_evento, '%d/%m/%Y') as fecha_evento, evento.nombre_evento, evento.invitados_evento, evento.tipo_evento, evento.cotizacion_evento, evento.notas_evento, evento.total_evento,estado_evento, (SELECT SUM(pagos.monto_pago) FROM pagos WHERE pagos.id_evento = evento.id_evento GROUP BY pagos.id_evento ) AS pagado, (SELECT SUM(detalle_evento.cantidad_detalle_evento * detalle_evento.precio_detalle_evento) FROM detalle_evento WHERE detalle_evento.id_evento = evento.id_evento GROUP BY pagos.id_evento ) AS total
			FROM evento
