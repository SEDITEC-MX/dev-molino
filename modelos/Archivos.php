...
*/
	...
	public function yaExisteArchivo($id_evento, $categoria_archivo){
		$arregloDatos = [$id_evento, $categoria_archivo];
		$sql="SELECT 1
			FROM archivos
			WHERE id_evento = ?
			AND categoriaArchivo = ?";
		return ejecutarSelect($sql, $arregloDatos);
	}

	public function insertarArchivo($id_evento, $nombre, $categoria_archivo)
	{
		$arregloDatos = [$id_evento, $nombre, $categoria_archivo];
		$sql="INSERT INTO archivos (id_evento, nombreArchivo, categoriaArchivo) 
		VALUES(?,?,?)";
		return ejecutarConsulta($sql, $arregloDatos);
	}

	public function mostrarArchivos($id_evento)
	{
		$arregloDatos = [$id_evento];
		$sql="SELECT 
		id_archivo,
		nombreArchivo, 
		categoriaArchivo,
	...
	public function borrarRegistroArchivo($id_evento, $categoria_archivo, $nombre_archivo)
	{
		$arregloDatos = [$id_evento, $categoria_archivo, $nombre_archivo];
		$sql="DELETE
		FROM archivos
		WHERE id_evento = ?
		AND categoriaArchivo = ?
	...
	public function mostrarArchivoIndividual($id_evento, $categoria_archivo)
	{
		$arregloDatos = [$id_evento, $categoria_archivo];
		$sql="SELECT 
		nombreArchivo 
