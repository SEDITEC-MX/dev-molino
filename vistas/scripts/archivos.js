$( document ).ready(function() {
	...
	$(".documentacion-subir").on("click",function(e){
        ...
        if(!tamanioArchivoEsValido(file, maxFileSize)){ //El parámetro maxsize está en MB
    ...
    $(".ver-archivo").on("click",  function(e) {
        let categoria = $(this).data("category");
        window.open("../ajax/archivos.php?op=descargarArchivo&id_evento=" + evento + "&categoria="+categoria);
    });
...
function mostrarArchivos(id_evento){
    $.post("../ajax/archivos.php?op=mostrarArchivos",{id_evento : id_evento}, function(data, status){
		data = JSON.parse(data);
