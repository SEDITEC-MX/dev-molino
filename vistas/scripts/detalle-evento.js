...
$(document).ready(function () {
  ...
  $("#btnExport").on("click", function (e) {
    ...
    var nombre_evento = [
      ["Nombre(s) del cliente(s)"],
      [$("#nombre_evento").val()],
    ];
    var invitados_evento = [
      ["Número de invitados"],
      [$("#invitados_evento").val()],
    ];
    var cotizacion_evento = [
      ["Tipo de cotización"],
...
function mostrar(id_evento) {
  ...
  $.post(
    "../ajax/evento.php?op=listarDetalleEvento&id=" + id_evento,
    function (r) {
      $("#detalles").html(r);
    }
  );
  ...
  $.post(
    "../ajax/evento.php?op=listarTablaAExportar&id=" + id_evento,
    function (r) {
      $("#sheetjs").html(r);
    }
  );
...
function actualizarTablaaExportar() {
  //Función para actualizar la tabla que exporta a excel. La invoca el botón de guardar
  // $("#detalles").
  $("#sheetjs > tbody").empty();
...
function agregarDetalle(id_servicio, nombre_servicio, categoria_servicio) {
  ...
  if (id_servicio != "") {
    ...
    var fila =
      ...
      cont +
      ')">X</button></td>' +
      '<td><input type="hidden" name="id_servicio[]" value="' +
      id_servicio +
      '">' +
      nombre_servicio +
      "</td>" +
      ...
      categoria_servicio +
      "</td>" +
      '<td><input type="number" class="form-control" name="cantidad_detalle_evento[]" id="cantidad_detalle_evento[]" value="' +
      cantidad_detalle_evento +
      '"></td>' +
      '<td><input type="number" class="form-control" name="precio_detalle_evento[]" id="precio_detalle_evento[]" value="' +
...
function modificarSubototales() {
  ...
  var sub = $("#detalles td span[name='subtotal[]']")
    .map(function () {
      return $(this);
    })
    .get();
...
$("#detallesCobros tbody").on("click", ".reenviar", function () {
  ...
  bootbox.confirm(
    ...
    function (result) {
      if (result) {
        ...
        {
          $.post(
            "../ajax/registro_cobro.php?op=reenviar_comprobante",
            {
              id_cobro: cobro,
              fecha_cobro: fecha,
              monto_cobro: monto,
...
$("#detallesCobros tbody").on("click", ".eliminar", function () {
  //Eliminar el cobro
  var boton = $(this);
  var cobro = $(this).parent().parent().attr("id");
  var fecha = $(this).parent().siblings(".fecha").text();
  ...
  bootbox.confirm("¿Está seguro de eliminar el cobro?", function (result) {
    if (result) {
      ...
      {
        $.post(
          "../ajax/registro_cobro.php?op=eliminar_cobro",
          {
            id_cobro: cobro,
            fecha_cobro: fecha,
            monto_cobro: monto,
...
$("#detallesPagos tbody").on("click", ".reenviar", function () {
  //Reenviar comprobante
  let boton = $(this);
...
$("#detallesPagos tbody").on("click", ".eliminar", function () {
  //Eliminar el pago
  let boton = $(this);
  let pago = $(this).parent().parent().attr("id");
  let fecha = $(this).parent().siblings(".fecha").text();
...
function listarCobrosPorMetodo() {
  ...
  const urlParams = new URLSearchParams(queryString);
  let id_evento = urlParams.get("evento");
  $.get("../ajax/detalle_evento.php?op=cobros&id=" + id_evento).done(function (
    data
  ) {
    data = JSON.parse(data);
    ...
    for (let i = 0; i < size; i++) {
      ...
      $("#cobrosPorMetodo tbody").append(
        "<tr><td></td><td>" + concatenar + "</td></tr>"
      );
    }
...
function listarServiciosPorCategoria() {
  ...
  const urlParams = new URLSearchParams(queryString);
  let id_evento = urlParams.get("evento");
  $.get("../ajax/detalle_evento.php?op=servicios&id=" + id_evento).done(
    function (data) {
      data = JSON.parse(data);
      let size = Object.keys(data).length;
      ...
      for (let i = 0; i < size; i++) {
        ...
        $("#serviciosPorCategoria tbody").append(
          "<tr><td></td><td>" + concatenar + "</td></tr>"
        );
...
function listarPagosPorCategoria() {
    ...
    const urlParams = new URLSearchParams(queryString);
    let id_evento = urlParams.get("evento");
    $.get("../ajax/detalle_evento.php?op=pagos&id=" + id_evento).done(function (
      data
    ) {
      data = JSON.parse(data);
      let size = Object.keys(data).length;
...
function calcularSaldoPendiente() {
  ...
  let pattern = /[^0-9.-]+/g;
  let saldo =
    parseFloat(servicios.replace(pattern, "")) -
...
function calcularUtilidad() {
    ...
    $("#tablaUtilidad > tbody > tr").each(function () {
      ...
      let utilidadFila =
        parseFloat(servicio.replace(pattern, "")) -
        parseFloat(pago.replace(pattern, ""));
...
function listarCategorias() {
  ...
  // const urlParams = new URLSearchParams(queryString);
  // let id_evento = urlParams.get('evento');
  $.get("../ajax/detalle_evento.php?op=listarCategorias").done(function (data) {
    data = JSON.parse(data);
    let size = Object.keys(data).length;
    ...
    for (let i = 0; i < size; i++) {
      ...
      $("#tablaUtilidad > tbody").append(
        "<tr id='" +
          ...
          "'>" +
          nombre_categoria_servicio +
          " </td><td>$0.00</td><td>$0.00</td><td>$0.00</td></tr>"
      );
    }
    // $("#tablaUtilidad > tbody").append('<tr ><td class="categoria no-definidos">Otros</td><td>$0.00</td><td>$0.00</td><td></td></tr>')

    $("#tablaUtilidad > tbody").append(
      '<tr class="totales"><td class="categoria-total">Total</td><td></td><td></td><td></td></tr>'
    );
    // let size = Object.keys(data).length
    // for(let i = 0; i < size; i++){
    // 	let precio = data[i]['monto'];
